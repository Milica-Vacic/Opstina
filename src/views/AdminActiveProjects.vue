<template>
  <div class="admin-active-projects-master">

    <div v-if="!projectToDisplay" class="all-active-projects-div">
      <h3>Активни Пројекти</h3>
      <p>Претрага: <input type="text" v-model="searchInfo" @keyup="getAllProjects"></p>
      <div v-for="(i,idx) in allProjects" :key="'project'+idx">
        ...
        <button @click="activeAreYouSure(i.pro_id)">Заврши Пројекат</button>
      </div>
    </div> 
...
export default {
  ...
  data(){
    return {
      ...
      areYouSureMsg: "Da li ste sigurni da zelite da okoncate projekat?",
      areYouSureData:null,
      errorPopup: false,
      errorMsgToShow: "",
  ...
  mounted(){
    ...
    if(selectedProject){
      this.getSingleProject(selectedProject);
    }
  },
  methods:{
    getAllProjects(){
